...
4. **Verify isolation — FAIL-FAST, before you branch or write anything.** Immediately after `aimfp_run` and **before** any `git_create_branch`, `reserve_file`/`reserve_function`, or any AIMFP write, call AIMFP `get_project_root()` and confirm it resolves **inside your own worktree** — it must match your cwd / the worktree path the master provisioned, **not** the shared main checkout. If it points at the shared main checkout (or anywhere outside your worktree), your branch and every tracking write would land in shared state and clobber it (this is the Run-2 failure). Do **not** branch, reserve, or write: send a `question` to the master — `intercomm_send(to: "master", message: "isolation check FAILED: get_project_root=<path>, expected my worktree <cwd>; not branching/writing", type: "question")` — and stop. Proceed only once `get_project_root()` is confirmed inside your worktree.
