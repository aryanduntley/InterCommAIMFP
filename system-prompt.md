...
> This section documents the core tools. The server exposes 18 tools total — the rest (`intercomm_assign`, `intercomm_get_protocol`, the orchestration tools `spawn`/`wake`/`scan`/`approve`/`teardown`, and the `worktree_*` registry) are covered in the Master Behavior and Worktree sections below.
